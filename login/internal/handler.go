...
func PlayerLogin(args []interface{}){
	...
	a := args[1].(gate.Agent)
	log.Debug("%v",a.RemoteAddr().String())
	if CheckPlayer(m.UserName,m.Pwd) {
	//有待改进
	    var UserId = Rand_User_id()
		a.WriteMsg(&p.LoginSuccess{
			...
			Result: struct {
				...
				IsRobt, IsGuest  bool
				State            int
			}{UserId:UserId,UserName:m.UserName,IsRobt:false,IsGuest:false , State: 1},
		})
	    notify.New(m.UserName,a)
		//UserSession.Store(UserId,p.C_LoginByPwd{UserName:m.UserName,Pwd:m.Pwd})
	}else {
